...
checkpoint = torch.load(Path('/gpfs/users/pellegrainv/RNN-Time-series-Anomaly-Detection/save/3tanks/checkpoint').joinpath(args_.filename).with_suffix('.pth'))
...
try:
    ...
    if 'means' in checkpoint.keys() and 'covs' in checkpoint.keys():
        ...
        mean, cov = checkpoint['means'], checkpoint['covs']
        #print(mean, cov)
    else:
    ...
    score, sorted_prediction, sorted_error, _, predicted_score = anomalyScore(args, model, test_dataset, mean, cov, score_predictor=score_predictor)
    ...
    '''
    values = test_dataset[0][0]
    for i in range(len(test_dataset))[1:]:
        values = torch.cat((values, test_dataset[i][0]), 0)
    target = preprocess_data.reconstruct(values.cpu(),
                                            test_dataset.trainset.mean,
                                            test_dataset.trainset.std).numpy()
    mean_prediction = preprocess_data.reconstruct(sorted_prediction.mean(dim=0).cpu(),
                                                    test_dataset.trainset.mean,
                                                    test_dataset.trainset.std).numpy()
    ...
    if args.save_fig:
        ...
        plt.savefig(str(save_dir.joinpath('fig_f_beta_channel').with_suffix('.png')))
        plt.close()
